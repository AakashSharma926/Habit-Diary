...
import { X, Eye, Search, Users } from 'lucide-react';
...
export function FriendsPanel({ onClose }: FriendsPanelProps) {
  const { allUsers, userProfile, isAuthenticated, signInWithGoogle, refreshUsers } = useAuth();
  const { setViewingUser, viewingUserId } = useHabits();
      ...
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className="bg-slate-800 rounded-2xl w-full max-w-md border border-slate-700 shadow-2xl">
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-slate-400 mb-6">Sign in to view other users' dashboards</p>
            <button
              onClick={signInWithGoogle}
              className="inline-flex items-center gap-3 px-6 py-3 bg-white hover:bg-gray-100 text-gray-800 rounded-xl font-medium transition-all shadow-lg"
            >
      ...
      <div className="bg-slate-800 rounded-2xl w-full max-w-lg border border-slate-700 shadow-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="p-6 border-b border-slate-700 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold">All Users</h2>
              <p className="text-sm text-slate-400">{filteredUsers.length} user{filteredUsers.length !== 1 ? 's' : ''}</p>
            </div>
          </div>
        ...
        <div className="p-4 border-b border-slate-700 shrink-0">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 bg-slate-700/50 border border-slate-600 rounded-xl focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 text-sm"
            />
          </div>
        </div>
        ...
        <div className="p-4 border-t border-slate-700 shrink-0">
          <p className="text-xs text-slate-400 text-center">
            View other users' dashboards - their data is read-only
          </p>
        </div>
      </div>
    </div>
